...
def save(model, global_step):
    ...
    if not os.path.exists(os.path.join(FLAGS.model_dir + '_' + FLAGS.run_id, 'saved_model')):
        os.mkdir(os.path.join(FLAGS.model_dir + '_' + FLAGS.run_id, 'saved_model'))
    export_dir = os.path.join(FLAGS.model_dir + '_' + FLAGS.run_id, 'saved_model')
...
def try_restore_from_checkpoint(model, global_step, optimizer):
    ...
    checkpoint_manager = tf.train.CheckpointManager(
        checkpoint,
        directory=FLAGS.model_dir + '_' + FLAGS.run_id,
...
def perform_evaluation(model, builder, eval_steps, ckpt, strategy, topology):
    ...
    ds = data_lib.build_distributed_dataset(builder, FLAGS.eval_batch_size, False,
                                            strategy, topology)
    if not os.path.exists(FLAGS.model_dir + '_' + FLAGS.run_id):
        os.mkdir(FLAGS.model_dir + '_' + FLAGS.run_id)
